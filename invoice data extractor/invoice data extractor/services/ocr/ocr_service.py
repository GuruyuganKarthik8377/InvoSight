...
def extract_text(file_path: str) -> dict:
    ...
    for page_idx, pil_image in enumerate(pages, start=1):
        try:
            np_img = pil_to_numpy(pil_image)

            # TASK 3: always compare raw vs preprocessed
            ...
            # Sharpen retry when preprocessed confidence is low
            if processed_result["confidence"] < _CONFIDENCE_SHARPEN_TH:
                try:
            ...
            # STEP 6 — per-page confidence log
            logger.info("[INFO] Page %d confidence: %.4f", page_idx, ocr_result["confidence"])
            if ocr_result["confidence"] < _CONFIDENCE_LOW_PAGE:
                logger.warning("[WARNING] Page %d confidence < 0.65: %.4f",
                               page_idx, ocr_result["confidence"])
